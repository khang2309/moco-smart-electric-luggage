...
const initialFilters: Filters = { range: "all", start: "", end: "", payment: "", status: "", shipping: "", product: "", customer: "" };
const currency = new Intl.NumberFormat("vi-VN", { style: "currency", currency: "VND", maximumFractionDigits: 0 });

const ranges = [
  ["all", "All time"], ["today", "Today"], ["yesterday", "Yesterday"], ["week", "This week"], ["lastWeek", "Last week"],
...
export default function AdminDashboard() {
  ...
  const displayMoney = (value: number | null | undefined) => value === null || value === undefined || !Number.isFinite(value) ? t.unavailable : currency.format(value);
    ...
    <div className="space-y-6 p-2 sm:p-4 lg:p-6">
      ...
      {loading && !data ? <DashboardSkeleton /> : error && !data ? <section className={`rounded-2xl border p-8 text-center shadow-sm ${panel}`}><h2 className="text-lg font-bold">{t.errorTitle}</h2><p className={`mt-2 text-sm ${muted}`}>{error}</p><button type="button" onClick={() => void loadDashboard()} className="mt-4 min-h-11 rounded-xl bg-blue-600 px-4 text-sm font-semibold text-white">{t.tryAgain}</button></section> : data && <>
        ...
        <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          <MetricCard label={language === "vi" ? "Tổng số đơn" : "Total orders"} value={String(data.kpis.totalOrders || 0)} detail={language === "vi" ? "Tổng tất cả trạng thái" : "All order statuses"} tone="slate" icon={ShoppingCart} />
