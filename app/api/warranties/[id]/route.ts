...
import { WARRANTY_COLLECTION, WARRANTY_EXTENSION_COLLECTION, WARRANTY_HISTORY_COLLECTION, serializeWarranty, synchronizeWarrantyStatus } from "@/lib/warranty";
...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const [history, extensions] = await Promise.all([
  ...
  return NextResponse.json({ success: true, warranty: serializeWarranty(await synchronizeWarrantyStatus(db, warranty)), history, extensions });
}
