  ...
  en: {
    ...
    content: {
      ...
      products: [
        {
          name: "MOCO Pro",
          description: "Upgraded battery, smoother speed, integrated GPS, and smart lock.",
          specs: ["Max speed 12 km/h", "10,000mAh battery", "20L capacity", "Simple controls"],
          featured: true,
        },
        {
          name: "MOCO Basic",
          description: "The standard version for essential daily mobility needs.",
        },
        {
          name: "Transparent Edition",
          description: "A transparent modern design with a futuristic look.",
        },
      ],
      features: [
...
async function getMocoContent(language: Language) {
  return pageCopy[language].content;
}

async function submitLead(language: Language) {
  await new Promise((resolve) => setTimeout(resolve, 500));
  return pageCopy[language].formThanks;
}
...
export default function Home() {
  ...
  return (
    <>
      <main>
        ...
        <section className="section split" id="about">
          ...
          <div className="mission-grid">
            {text.about.cards.map(([title, description]) => (
              <article key={title}>
                <h3>{title}</h3>
                <p>{description}</p>
              </article>
        ...
        <section className="section product" id="product">
          ...
          <div className="product-layout">
            {data.products.map((product) => {
              ...
              const specs = "specs" in product ? product.specs : null;

              return (
              <article key={product.name} className={`product-card${isFeatured ? " featured" : ""}`}>
                {isFeatured && <span className="badge">{text.product.badge}</span>}
                <h3>{product.name}</h3>
        ...
        <section className="section use-cases" id="use-cases">
          <div className="section-heading">
            <p className="eyebrow">{text.headings.useCasesEyebrow}</p>
            <h2>{text.headings.useCases}</h2>
          </div>
      ...
      <footer className="site-footer about-footer" id="footer-about">
        <div className="footer-about-hero">
          <div className="footer-silhouettes" aria-hidden="true">
            <span></span>
            <span></span>
            <span></span>
            <span></span>
        ...
        <div className="footer-about-content">
          <article>
            <h2>{text.footer.storyTitle}</h2>
            {text.footer.story.map((paragraph) => (
              <p key={paragraph}>{paragraph}</p>
            ))}
          </article>
          <article>
            <h2>{text.footer.visionTitle}</h2>
            <p>{text.footer.vision}</p>
          </article>
          <article>
            <h2>{text.footer.missionTitle}</h2>
            <p>{text.footer.mission}</p>
          </article>
          <article>
            <h2>{text.footer.teamTitle}</h2>
            <p>{text.footer.team}</p>
          </article>
        </div>
      </footer>
    </>
